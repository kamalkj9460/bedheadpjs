...
(function () {
  ...
  ProductGridItem.prototype.compileTemplate = function (data, index, totalProduct) {
    ...
    // Add promotion card
    var positionCard = boostPFSThemeConfig.custom.position_of_promotion_card - 1;
		var collection_handle_arr = boostPFSThemeConfig.custom.collection_handle.split(',')|| [];
    if (index == positionCard && collection_handle_arr.includes(boostPFSConfig.general.collection_handle) && boostPFSThemeConfig.custom.image_promotion !== '') {
      itemHtml = boostPFSTemplate.promotionalCardHtml + itemHtml;
      itemHtml = itemHtml.replace(/{{promotionUrl}}/g, boostPFSThemeConfig.custom.collection_link_url);
      var imagePromotionHtml = '<div class="image-element__wrap" style="">' +
      '<img' +
      ' class="lazyload transition--' + boostPFSConfig.custom.image_loading_style + ' "' +
      ' src="' + boostPFSThemeConfig.custom.image_promotion + '"' +
      ' data-src="' + Utils.optimizeImage(boostPFSThemeConfig.custom.image_promotion, '480x') + '"' +
      ' data-sizes="auto"' +
      ' srcset="'+ boostPFSThemeConfig.custom.image_promotion +'"' +
      '/>' +
      '</div>';
      itemHtml = itemHtml.replace(/{{promotionImages}}/g, imagePromotionHtml);
      itemHtml = itemHtml.replace(/{{promotionInfo}}/g, boostPFSThemeConfig.custom.promotion_title);
    }

   
    // add custom code
     var video_class = '';
     if (boostPFSConfig.custom.show_secondary_image && data.media[1] && data.media[1].mediaContentType === 'VIDEO') {
        video_class = ' onhover_video ';
     }
    ...
    // add custom code
     //itemDesktopWidthClass += video_class;
  ...
  function buildStickers(data) {
    ...
    if (boostPFSConfig.custom.stickers_enabled) {
      ...
      var stickerCollections = ['best-seller', 'coming-soon', 'new', 'pre-order', 'staff-pick', 'final-sale', 'warehousesale'];
       var WarehouseSale = true;
        // add custom code 
        for (var i = 0; i < stickerCollections.length; i++) {
          var stickerCollection = stickerCollections[i];
           ...
           // add custom code 
            var sale = stickerCollection.replace('-', '_'); 
             var stickerText = '', addclass='';
             if(sale == 'warehousesale'){
                stickerText = 'Warehouse Sale';
               addclass = 'WarehouseSale';
               WarehouseSale = false;
                var sticker = '<div class="' + stickerCollection + '-sticker thumbnail-sticker sticker-' + i + ' ' +addclass+'"><span class="sticker-text">' + stickerText + '</span></div>';
            ...
            var stickerCollection = stickerCollections[i];
             ...
             // add custom code 
              var sale = stickerCollection.replace('-', '_'); 
               var stickerText = '', addclass='';
               if(sale == 'final_sale'){
                  stickerText = 'Final Sale';
               }else{
                   stickerText = boostPFSConfig.label[stickerCollection.replace('-', '_')];
               } 
             
             if(typeof stickerText !== 'undefined'){
                var sticker = '<div class="' + stickerCollection + '-sticker thumbnail-sticker sticker-' + i + ' ' +addclass+'"><span class="sticker-text">' + stickerText + '</span></div>';
                stickers.push(sticker);
             }
            }
  ...
  function buildImages(data) {
    ...
    if(plus_size){
      ...
      $.each(images, function(index, item){
        if(item.src.indexOf('plus_size') > 0 && count){
           img = images[index];
          count = 0;
         }
      });
   ...
   // console.log(images[0]);
    ...
    /* 
    if (boostPFSConfig.custom.show_secondary_image && data.media[1] && data.media[1].mediaContentType === 'VIDEO') {
      html += '<div class="product-thumbnail__video secondary swap--visible video-on-hover" data-html5-video>';
      html += '  <video autoplay loop muted playsinline controls="false" class="product-video">';
      data.media[1].sources.forEach(function(source) {
        var mimeType = source.mimeType;

        if (!mimeType) {
          if (source.url.includes('.m3u8')) mimeType = 'application/x-mpegURL';
          if (source.url.includes('.mp4')) mimeType = 'video/mp4';
        }
        
        html += '    <source src="' + source.url + '" type="' + mimeType +  '">';
      });      
      if (data.media[1].preview && data.media[1].preview.image) {
        html += '  <img src="' + data.media[1].preview.image.originalSrc + '" role="presentation" alt="" data-uw-rm-ima="un" class="">';
      }
      html += '  </video>';            
      html += '</div>';      
    } else 
  ...
  function buildImageElement(image, additionalClass) {
    ...
    html += '<div class="image-element__wrap" style="' + backgroundColor + heightStyle + widthStyle + '">' +
      ...
      (lowQualityImage && lowQualityImage !== '' ?
        ' src="' + lowQualityImage + '"' : '') +
      ' data-src="' + Utils.optimizeImage(image.src, '480x') + '"' +
      ' data-sizes="auto"' +
