...
window.PXUTheme.jsAjaxCart = {
  ...
  removeFromCart: function (lineID, callback) {
    $.ajax({
      ...
      success: function (cart) {
		...
		window.PXUTheme.jsAjaxCart.initializeAjaxCart();
      },
  ...
  add_custom_code: function(html){
     // add on full function 
        $("html, body").animate({ 
         		//scrollTop: 0
       }, 1000); 
            $.ajax({
              ...
              success: function (cart) {
                 //  console.log(cart);
                // hide gift in cart 
                $('.ajax-cart__list .ajax-cart__product').each(function(){
                  var string = $(this).data('cart-item');
                  if(string.indexOf('7259307802652') != -1){
                    $(this).hide();
                  }                  
                });
                ...
                $.each(cart.items, function(index, item){
                  ...
                  var gift_txt = cart.attributes[g_name], show_gift = false, g_class = '', show = '';
                  var input = '<input class="js-giftwrap" data-qty="'+item.quantity+'" data-val="'+g_name+'" type="checkbox" id="Complimentary-Gift-checkbox">';
                 
                  
                  if(typeof gift_txt !== 'undefined' && gift_txt != 'undefined'){
                    
                    show_gift = true;
                    g_class = ' active';
                    input = '<input class="js-giftwrap" '+item.quantity+' data-val="'+g_name+'" type="checkbox" checked id="Complimentary-Gift-checkbox">';
                    show = 'style="display:block"';
                  }else{
                    gift_txt = '';
                  }
                  
                  var gift_input =' <div class="giftwrap-option '+g_class+'">'+
                     
                         input+
                        '<label for="Complimentary-Gift-checkbox">Complimentary Gift Box</label>'+
                      '</div>'+
                      '<div class="gift-note-field" '+show+'>'+
                        '<label for="gift-note">Gift Note (Optional)</label>'+
                        '<textarea id="gift-note" data-val="Gift Note for '+item.title+'" placeholder="For my dearest...">'+gift_txt+' </textarea>'+
                      '</div>';
                  var line = index + 1; 
                  // $('[data-line-item="'+line+'"] .ajax-cart__product-content').append(gift_input);
                  $('[data-line-item="'+line+'"] .ajax-cart__quantity-warning').html(gift_input);
                });
    ...
    $('.ajax-cart__details-wrapper').prepend(crocsSell);
    // $('.ajax-cart__product-content').append(gift_input);
